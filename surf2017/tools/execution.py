...
from nntrader.trade.papertrader import PaperTrader
...
def execute_paper_trading(algo, config, total_steps):
    """
    @:param algo: string representing the name the name of algorithms
    """
    agent, agent_type, net_dir = _construct_agent(algo)
    PaperTrader(config, total_steps, agent=agent,
                agent_type=agent_type, net_dir=net_dir).start_trading()
...
def execute_backtest(algo, config):
    ...
    return backtester.validation_pc_vector
