...
def get_test_data(config):
    ...
    config["input"]["global_period"] = config["input"]["trade_period"]
    price_matrix = DataMatrices.create_from_config(config)
    cv_set = price_matrix.get_cross_validation_set()["y"][:, 0, :].T
    cv_set = np.concatenate((np.ones((1, cv_set.shape[1])), cv_set), axis=0)
    return cv_set
