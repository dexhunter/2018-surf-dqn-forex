...
def fill_input_default(input_config):
    set_missing(input_config, "fake_data", False)
...
def fill_layers_default(layers):
    for layer in layers:
        ...
        elif layer["type"] == "SingleMachineFully":
            set_missing(layer, "activation_function", "relu")
            set_missing(layer, "regularizer", None)
            set_missing(layer, "weight_decay", 0.0)
        elif layer["type"] == "FullyLayer":
            set_missing(layer, "activation_function", "relu")
            set_missing(layer, "regularizer", None)
            set_missing(layer, "weight_decay", 0.0)
        elif layer["type"] == "SingleMachineLSTM" or layer["type"] == "SingleMachineRNN":
            set_missing(layer, "dropouts", None)
        elif layer["type"] == "SingleMachineOutput" or layer["type"] == "SingleMachineOutput_WithW":
            set_missing(layer, "regularizer", None)
            set_missing(layer, "weight_decay", 0.0)
        else:
