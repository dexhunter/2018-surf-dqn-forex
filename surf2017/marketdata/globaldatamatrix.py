...
class HistoryManager:
    ...
    def get_global_data_matrix(self, start, end, period=300, features = ('close',)):
        ...
        """
        return self.get_global_dataframe(start, end, period, features).to_panel().as_matrix().transpose(1,0,2)
    ...
    def get_global_dataframe(self, start, end, period=300, features=('close',)):
        ...
        :return a mutiIndex dataframe, the index is [feature, time], column is coin
        """
        ...
        index = pd.MultiIndex.from_product([features, time_index], names=["feature_name", "date_norm"])
        dataframe = pd.DataFrame(index=index, columns=coins, dtype=np.float32)
            ...
            for row_number, coin in enumerate(coins):
                for feature in features:
                    ...
                    serial_data = pd.read_sql_query(sql, con=connection,
                                                    ...
                                                    index_col="date_norm")
                    serial_data.columns = [coin]
                    logging.debug("get data of {coin}, {feature}"
                                  .format(coin=coin, feature=feature))
                    dataframe.loc[feature][coin]\
                        .loc[serial_data.index] = serial_data[coin]
        finally:
            connection.commit()
            connection.close()
        return dataframe
    ...
    def select_coins(self, start, end):
        if not self._online:
            ...
            coins = self._coin_list.topNVolume(n=self._coin_number).index
...
class FakeHistoryManager:
    ...
    def get_global_data_matrix(self, start, end, period=300):
        time_step = int((end-start) / period)
        data = np.empty(shape=(1, self.__coin_number, time_step))
        param_list = [np.random.random() for i in range(5)]
        for i in range(self.__coin_number):
            data[:,i,:] = self.random_sequence(time_step, param_list)
        return data

    @staticmethod
    def random_sequence(length, sin_params, fluctuation_ratio=0.015):
        def generate_function(index_array):
            phase = np.random.randint(0, 50)
            for i in index_array:
                i = int(i)
                index_array[i] = 1 + fluctuation_ratio*(sum([sin((i+phase)*param) for param in sin_params]))
            return index_array
        return np.fromfunction(function=generate_function,
                               shape=(length,))
