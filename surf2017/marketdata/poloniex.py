...
class Poloniex:
    ...
    def get_market_price(self, type, pair):
        orders = self.marketOrders(pair=pair)
        if type == "buy":
            return float(orders["asks"][0][0])
        elif type == "sell":
            return float(orders["bids"][0][0])
        else:
            raise ValueError("type " + type + " is not supported")
    ...
    def api(self, command, args={}):
        ...
        if command in PRIVATE_COMMANDS:
            ...
            # TODO: deal with time out error, or think ahout asynchronous structure
